...
This document provides detailed sequence diagrams for the key API interactions in the TAINA Voice AI system. It focuses on the communication patterns between the Voice AI UI (frontend), WebRTC Agent (backend), and external services such as TTS, TTS, OpenAI, and the appointment booking system.

## 1. Speech-to-Text Flow (TTS API)

The following sequence diagram illustrates the flow of audio from the user through the system to the TTS API for speech recognition:
...
## 2. Text-to-Speech Flow (TTS API)

The following sequence diagram illustrates how the system generates speech from text using the TTS API:
...
## 5. WebRTC WebRTC Flow

The following sequence diagram illustrates the WebRTC connection flow using WebRTC:
